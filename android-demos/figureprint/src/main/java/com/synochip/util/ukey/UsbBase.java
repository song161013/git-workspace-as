package com.synochip.util.ukey;
...
import com.synochip.util.Utils;

public class UsbBase {
    private static final String TAG = "usbBase";
...
//	private  UsbEndpoint mPointInterrupt;
    ...
    public int close() {
        if (null != mConnection) {
//            if (!mConnection.releaseInterface(mIntf)) {
                ;// throw  new IllegalArgumentException("connection releaseInterface error");
                Log.e(TAG, "AA connection releaseInterface error");
//            }
            mConnection.close();
            mConnection = null;
    ...
    public int open() {
        ...
        Log.i(TAG, "AA endpoint out count = " + count);
...
//			}
    ...
    public synchronized int synoDownData(byte[] sendBuf, int sendBufLen) {
		...
		do_CBW[6] = 'n';
		do_CBW[7] = 'o';
		do_CBW[8] = (byte)(sendBufLen & 0xff);
		do_CBW[9] = (byte)((sendBufLen>>8) & 0xff);
		do_CBW[10]= (byte)((sendBufLen>>16)& 0xff);
		do_CBW[11]= (byte)((sendBufLen>>24)& 0xff);
		do_CBW[12]= (byte)0x00;
		do_CBW[13]= (byte)0x00;
		do_CBW[14]= (byte)0x0a;
		do_CBW[15]= (byte)0x86;
		...
		if(ret != UsbStatus.RETURN_SUCCESS){
			return 	-11;//UsbStatus.RETURN_ERROR; 
		}
		//send Data
		ret = _Write(sendBuf , sendBufLen);
		if(ret != UsbStatus.RETURN_SUCCESS){
			return 	-12;//UsbStatus.RETURN_ERROR; 
		}
		...
		if((di_CSW[3] != 0x53) || (di_CSW[12] != 0x00)){
			return -508;//UsbStatus.RETURN_ERROR;
		}
		di_CSW[3] = 0x43;
	...
	*/
    ...
    public synchronized int scsi_recv_1(byte[] receiveBuf, int[] receiveBufLen) {
        ...
        String test = Utils.bytesToHexString(bycbw, bycbw.length);
        ret = _Write(bycbw, 31);
        if (ret != UsbStatus.RETURN_SUCCESS) {
            return UsbStatus.RETURN_ERROR;
        }
        ret = _Read(tmpReceiveBuf, receiveLen);
        String test1 = Utils.bytesToHexString(tmpReceiveBuf, tmpReceiveBuf.length);
        if (ret != UsbStatus.RETURN_SUCCESS) {
            return UsbStatus.RETURN_ERROR;
        }
    ...
    public synchronized int scsi_recv_Image(byte[] receiveBuf, int len, int offset) {
        ...
        ret = _Write(bycbw, 31);
        String upImageOrder = Utils.bytesToHexString(bycbw, bycbw.length);
        if (ret != UsbStatus.RETURN_SUCCESS) {
            return -901;
        }
...
*/
        ...
        if (length[0] >= 36864) {
            ...
            for (int i = 0; i < 4; i++) {
                try {
                    ...
                    Log.i(TAG, "CC ength[0] >= 36864 读到指令=" + Utils.bytesToHexString(tempBuf, tempBuf.length));
                } catch (Exception e) {
                    return UsbStatus.RETURN_ERROR;
                }
            }
            ...
            for (int i = 0; i < 2; i++) {
                try {
                    ...
                    System.arraycopy(tempBuf, 0, buffer, len * i, len);
                    Log.i(TAG, "CC length[0] >= 16384 && length[0] < 36864 读到指令=" + Utils.bytesToHexString(tempBuf, tempBuf.length));
                } catch (Exception e) {
                    return UsbStatus.RETURN_ERROR;
                }
            }
            ...
            Log.i(TAG, "CC  else 读到指令=" + Utils.bytesToHexString(buffer, buffer.length));
            System.out.println("read 4444 len " + len);
            if (len < 0) {
                return UsbStatus.RETURN_ERROR;
            }
    ...
    private static int _Write(byte[] buffer, int length) {
        ...
        try {
            ...
            Log.i(TAG, "CC 写入指令=" + Utils.bytesToHexString(buffer, buffer.length));
            mConnection.bulkTransfer(mPointOut, buffer, length, 30000);
     ...
     * @param length  长度
     */
    public int writeData(byte sendBuf[], int length) {
        int ret = -1;
        try {
            ret = mConnection.bulkTransfer(mPointOut, sendBuf, length, 30000);
        } catch (Exception ex) {
            return UsbStatus.RETURN_ERROR;
        }
        return ret;
     ...
     * 读数据
     *
     * @param receiverBuf 读数据
     * @param length      长度
     */
    private int readData(byte receiverBuf[], int length) {
        int len;
        try {
            len = mConnection.bulkTransfer(mPointIn, receiverBuf, receiverBuf.length, 60000);
        } catch (Exception ex) {
            Log.i(TAG, "AA ex readdata exception " + ex);
            return UsbStatus.RETURN_ERROR;
        }
        return len;
    }

    public synchronized int getImageData(byte sendBuf1[], byte sendBuf2[], byte receiverBuf[], int length) {
        int ret1, ret2;
        ret1 = writeData(sendBuf1, sendBuf1.length);
        String Buf1 = Utils.bytesToHexString(sendBuf1, sendBuf1.length);
        Log.i(TAG, "AA 指纹发送 " + Buf1 + ",返回值=" + ret1);

        ret2 = writeData(sendBuf2, sendBuf2.length);
        String Buf2 = Utils.bytesToHexString(sendBuf2, sendBuf2.length);
        Log.i(TAG, "AA 指纹发送 " + Buf2 + ",返回值=" + ret2);

        int redret1 = readData(receiverBuf, length);
        String receibyte1 = Utils.bytesToHexString(receiverBuf, receiverBuf.length);
        Log.i(TAG, "AA 指纹读 " + receibyte1 + ",返回值=" + redret1);

//        int redret2 = readData(receiverBuf, length);
//        String receibyte2 = Utils.bytesToHexString(receiverBuf, receiverBuf.length);
//        Log.i(TAG, "AA 指纹读 " + receibyte2 + ",返回值=" + redret2);

        if (receiverBuf[10] == 0) {
            return Constants.INPUT_FINGER_SUCCESS;
        } else if (receiverBuf[10] == 1) {
            return Constants.RECEIVER_KIT_WRONG;
        } else if (receiverBuf[10] == 2) {
            return Constants.NO_FINGER_PRINT;
        } else if (receiverBuf[10] == 3) {
            return Constants.NPUT_FINGER_FAILED;
        }

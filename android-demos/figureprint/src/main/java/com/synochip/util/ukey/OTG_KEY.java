...
public class OTG_KEY {
    private static final String TAG = "OTG_KEY";
    ...
    public final static byte DELETE_GEN_CHAR = 0x0c;
    public final static byte SEARCH = 0x04;

    public final static byte BURN_CODE = 0x1a;
    public final static byte DOWN_IMAGE = 0x0b;
    ...
    public boolean verifySignedData(byte[] cert, byte[] inData, byte[] signedValue, int hashType) {

        return true;

    }

    public boolean importCertificate(String containerName, int keyType, byte[] cert) {
        byte[] bhContainer = new byte[2];
        if (!misOpen) {
            return false;
        }
     ...
     * @param pContent p
     */
    //add by syno finger usb  // num = fillPackage(sendData, CMD, 1, cmd);
    public int fillPackage(byte[] sendData, int packageType, int length, byte[] pContent) {
        int totalLen = 0;
        ...
        length += 2;
        sendData[0] = (byte) packageType;
        sendData[1] = (byte) ((length >> 8) & 0xff);
        sendData[2] = (byte) (length & 0xff);

        System.arraycopy(pContent, 0, sendData, 3, length);
    ...
    public int EnCode(int addr, byte[] source, int sourceLen, byte[] destination /*目的地*/, int[] destinationLength) {
        int i = 0, chkSum = 0;
        int ValH, ValL;
    ...
    public int GetPackageUDisk(byte[] data) {
        ...
        if ((ret = msyUsbBase.synoGetData(recvBuf, len)) != 0) {//scsi_recv_1
            return ret;
        }
        if ((ret = DeCodeUSB(recvBuf, len[0], data, desLen)) != 0) {
            return ret;
    ...
    public int GetPackageImageUDisk( byte[] data, int Imagelen )
    {
        ...
        {
            j=i*iTmpLen;
            if( (ret = msyUsbBase.scsi_recv_Image(data,iTmpLen,j)) != 0){
                return ret;
            }
        }
    ...
    */
    ...
    public int PSGetImage(int addr) {
        ...
        String s1 = Utils.bytesToHexString(sendData, sendData.length);
        Log.i(TAG, "BB senddata1=" + s1);
        if ((ret = SendPackageUDisk(addr, sendData)) != 0) {
            return -1;
        }

        String s2 = Utils.bytesToHexString(sendData, sendData.length);
        Log.i(TAG, "BB senddata2=" + s1);

        if (0 != GetPackageUDisk(getData)) {
            return -2;
        }
        String s3 = Utils.bytesToHexString(getData, getData.length);
        Log.i(TAG, "BB getData1=" + s3);

        result = VerifyResponsePackage(RESPONSE, getData);

        String s4 = Utils.bytesToHexString(getData, getData.length);
        Log.i(TAG, "BB getData2=" + s4);
        return result;
    }
    ...
    public int PSStoreChar(int addr, int bufferID, int pageID) {
        ...
        cmd[0] = STORE_CHAR;//DELETE_GEN_CHAR
        cmd[1] = (byte) bufferID;
     ...
     * @param count  删除的模板个数
     */
    public int PSdeleteGenChar(int addr, int pageID, int count) {
        byte[] cmd = new byte[10];
        int num;
        int result;
        int ret;
    ...
    public int PSSearch(int addr, int bufferID, int startPage, int pageNum, int[] mbAddr) {
        ...
        num = fillPackage(sendData, CMD, 6, cmd);//将cmd打包给sendData
        if ((ret = SendPackageUDisk(addr, sendData)) != 0) {
            return -1;
        }
    ...
    public int PSUpImage(int addr, byte[] pImageData) {
        ...
        num = fillPackage(sendData, CMD, 1, cmd);
        String receibyte = Utils.bytesToHexString(sendData, sendData.length);
        if ((ret = SendPackageUDisk(addr, sendData)) != 0) {
            return -56;
        }
    ...
    //GetPackageUDisk
    public int FP_GetImage(byte sendBuf1[], byte sendBuf[], byte receiverBuf[], int length) {
        return msyUsbBase.getImageData(sendBuf1, sendBuf, receiverBuf, length);
    }
    ...
    public int searchFingerPrint(int addr, int BufferID, int StartPage, int PageNum) {
        byte[] cmd = new byte[10];
        int num;
        int result;
        int ret;
    ...
    public int PSDownImage(int addr) {
        byte[] cmd = new byte[10];
        int num;
        int result;
        int ret;
    ...
    public void sendDataPacket(byte data[]) {
        int ret = msyUsbBase.writeData(data, data.length);
//        String Buf1 = Utils.bytesToHexString(data, data.length);
        int[] len = new int[1];
        len[0] = 64;
        if ((ret = msyUsbBase.synoGetData(data, len)) != 0) {//scsi_recv_1
//            return ret;
        }
//        Log.i(TAG, "AA 发送数据包 " + Buf1);
        Log.i(TAG, "AA 返回值=" + ret);
    }
